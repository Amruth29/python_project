    ...
    def game_over():
        ...
        with open("High_Score.txt", "w") as file:
            if score > int(high_score):
                high_score = score
                canvas4.create_text(800, 400, text=f"Congratulations {name}! You have set a new high score",
        ...
        with open("Name.txt", "r") as c:
            ...
            if name in name_list:
                ...
                with open("Score_list.txt", "r") as d:
                    score_list = list(map(lambda z: int(z.strip()), d.readlines()))
                    score_list[k] += score
                with open("Score_list.txt", "w") as e:
                    for m in score_list:
                        e.write(f"{str(m)}\n")
            else:
...
def leaderboard():
    ...
    for p in scores_list_sort:
        canvas8.create_text(500, space, text=names_list[scores_list.index(p)], font=("Courier", 30, "bold"), fill="white")
